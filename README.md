# 유가로 원/달러 환율을 예측할 수 있는가?

> 국제유가로 원/달러 환율을 예측할 수 있는지 통계적으로 검증한 프로젝트.
> 결론은 **예측할 수 없다**이며, 검증 과정에서 확인된 약한 동시점 관계를 **유가 시나리오별 환율 민감도 분석 대시보드**로 정리했다.
...
1. **유가로 환율을 예측할 수 없다.** 전날까지의 유가로 오늘 환율을 설명하는 힘은 0.13%다.
   그레인저 검정은 통계적으로 유의했지만(p=0.006) 경제적으로는 의미가 없는 크기다.
2. **같은 날 함께 움직이는 관계는 있지만, 약하고 불안정하다.** 2010~2024년에는 유가가 오를 때 원화가 강해졌고(상관 -0.10),
   2025년 이후에는 반대로 약해진다(+0.27). QLR 검정이 **2025-04-29**를 구조변화 시점으로 찾아냈다.
3. **그래서 대시보드는 예측 도구가 아니라 민감도 분석 도구다.** 유가 경로를 *가정*하면 과거 관계를 적용해 환율 범위를 계산한다.
   Brent +20%(20영업일)를 가정해도 과거 관계로 설명되는 환율 변화는 +12원이고, 설명되지 않는 변동은 ±76원(95%)이다.

---

## 1. 문제 정의
- **출발 가설:** 한국은 원유 순수입국이다. 유가가 오르면 무역수지가 나빠지고 달러 수요가 늘어 **원화가 약해질 것이다** (무역수지 경로).
- **반대 가능성:** 세계 경기가 좋을 때는 유가 상승과 위험자산 선호가 함께 나타나 **원화가 강해지기도 한다** (글로벌 경기 경로).
- **원래 목표:** 두 변수의 상관관계를 확인하고, 관계가 확인되면 **유가 변동으로 환율 변동을 예측하는 그래프**를 만든다.
- **검증 질문:** ① 어느 경로가 우세한가? ② 관계는 안정적인가? ③ 유가로 환율을 예측할 수 있는가?

## 원래 목표와 달라진 점
| | 원래 목표 | 검증 결과 | 대응 |
|---|---|---|---|
| 상관관계 | 유가↑ → 원화↓ 확인 | 전체 기간은 **반대 방향**, 2025년 이후에만 가설 방향 | 국면별 분석과 구조변화 검정으로 전환 시점을 확인 |
| 예측 | 유가로 환율 예측 그래프 | **예측력 없음** (설명력 0.13%) | 목표를 억지로 유지하지 않고 결론을 "예측 불가"로 정리 |
| 대시보드 | 환율 예측 그래프 | – | 남은 동시점 관계를 **민감도·스트레스 분석**으로 제공 |

- 환율이 경제 변수로 예측되지 않는다는 결과는 국제금융에서 오래된 발견이다. Meese & Rogoff(1983) 이후 대부분의 환율 모형은
  표본 밖에서 "내일 환율 = 오늘 환율"이라는 단순 예측을 이기지 못했다. 이 프로젝트는 유가 하나로 같은 결과를 한국 데이터에서 재확인했다.
- 대시보드의 헤지 계산기는 이 결과를 실무에 쓰는 **한 가지 예시**다. 헤지가 최적 전략이라고 주장하는 것은 아니다.
...
- 전 기간 하나의 계수를 쓰면 현재 국면을 잘못 반영한다.
...
> 이 검증은 **그날의 실제 유가 변화를 알려준 상태에서** 환율 변화를 얼마나 설명하는지 본다. 즉 예측력이 아니라
> *유가 경로를 가정했을 때 계산되는 범위가 믿을 만한지*를 검증한다. 유가를 미리 알 수 없다면 이 설명력도 쓸 수 없다.
...
| 기능 | 내용 |
|---|---|
| 시나리오 분석 | Brent 변화율·기간을 **가정**하고 β 방식을 고르면 원/달러 조건부 중심과 80%/95% 구간 |
| 과거 충격 프리셋 | 2014 급락, 2020 코로나, 2022 전쟁, 2026 급등을 오늘에 적용하고, **당시 데이터만으로 계산한 범위와 실제 결과**를 비교 |
...
## 6. 시사점과 향후 과제
- **실무 시사점:** 유가 전망만으로는 환율 방향을 판단할 근거가 약하다. 유가에 노출된 기업이라면 유가와 환율을 별개의 위험으로 보고 각각의 범위를 관리하는 편이 합리적이다.
